...
function Progress(action, finish) Process(action, nil, nil, finish) end
function ProgressWithStartEvent(action, start, finish) Process(action, start, nil, finish) end
function ProgressWithTickEvent(action, tick, finish) Process(action, nil, tick, finish) end
function ProgressWithStartAndTick(action, start, tick, finish) Process(action, start, tick, finish) end
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            isAnim = false
            isProp = false
            SendNUIMessage({action = 'mythic_progress',duration = mythic_action.duration,label = mythic_action.label})
            Citizen.CreateThread(function ()
                if start ~= nil then start() end
                while isDoingAction do
                    Citizen.Wait(1)
                    if tick ~= nil then tick() end
                    if IsControlJustPressed(0, 178) and mythic_action.canCancel then TriggerEvent('mythic_progbar:client:cancel') end
                    if IsEntityDead(PlayerPedId()) and not mythic_action.useWhileDead then TriggerEvent('mythic_progbar:client:cancel') end
                end
                if finish ~= nil then finish(wasCancelled) end
            end)
        else
        ...
        while runProgThread do
            if isDoingAction then
                if not isAnim then
                    if mythic_action.animation ~= nil then
                        ...
                        elseif mythic_action.animation.animDict ~= nil and mythic_action.animation.anim ~= nil then
                            if mythic_action.animation.flags == nil then mythic_action.animation.flags = 1 end
                ...
                if not isProp and mythic_action.prop ~= nil and mythic_action.prop.model ~= nil then
                    ...
                    while not HasModelLoaded(GetHashKey(mythic_action.prop.model)) do Citizen.Wait(0) end
                    ...
                    if mythic_action.prop.bone == nil then mythic_action.prop.bone = 60309 end
                    if mythic_action.prop.coords == nil then mythic_action.prop.coords = {x = 0.0, y = 0.0, z = 0.0} end
                    if mythic_action.prop.rotation == nil then mythic_action.prop.rotation = {x = 0.0, y = 0.0, z = 0.0} end
                    ...
                    if not isPropTwo and mythic_action.propTwo ~= nil and mythic_action.propTwo.model ~= nil then
                        ...
                        while not HasModelLoaded(GetHashKey(mythic_action.propTwo.model)) do Citizen.Wait(0) end
                        local pCoords = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)
                        ...
                        NetworkSetNetworkIdDynamic(netid, true)
                        SetNetworkIdCanMigrate(netid, false)
                        if mythic_action.propTwo.bone == nil then mythic_action.propTwo.bone = 60309 end
                        if mythic_action.propTwo.coords == nil then mythic_action.propTwo.coords = {x = 0.0, y = 0.0, z = 0.0} end
                        if mythic_action.propTwo.rotation == nil then mythic_action.propTwo.rotation = {x = 0.0, y = 0.0, z = 0.0} end
...
function Cancel()
    ...
    wasCancelled = true
    ActionCleanup()
    SendNUIMessage({action = 'mythic_progress_cancel'})
end
